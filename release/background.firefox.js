...
async function cleanStorage(beginKey, endKey) {
    if (isSafari) {
        await browser.runtime.sendNativeMessage("OpenTheatre.OpenTheatre",
            {
                source: "OpenTheatre",
                type: 3009,
                id: generateUUID(),
...
async function estimateStorageUsage() {
    if (isSafari) {
        const resp = await browser.runtime.sendNativeMessage("OpenTheatre.OpenTheatre",
            {
                source: "OpenTheatre",
                type: 3007,
                id: generateUUID(),
...
const chunkNumMagic = "OpenTheatreChunkNumber:"
...
function openDB() {
    const openRequest = indexedDB.open("OpenTheatre", 9);
