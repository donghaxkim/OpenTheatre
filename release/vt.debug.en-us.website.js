...
// @version      1779383803
// @description  Watch videos together with friends on any site
...
(function () {
    ...
    function updateInnnerHTML(e, html) {
        try {
            ...
            if (trustedPolicy == undefined) {
                trustedPolicy = trustedTypes.createPolicy('openTheatreExtensionVtJsPolicy', {
    ...
    function downloadEnabled() {
        try {
            if (window.OpenTheatreDownload == 'disabled') {
                return false;
            }
            const type = OpenTheatreStorage.UserscriptType
            return parseInt(window.OpenTheatreStorage.LoaddingVersion) >= 1694758378
    ...
    function startDownload(_vtArgM3u8Url, _vtArgM3u8Content, _vtArgM3u8Urls, _vtArgTitle, _vtArgPageUrl) {
        // download removed (was VideoTogether feature)
    }
    ...
    function getOpenTheatreStorage(key, defaultVal) {
        try {
            if (window.OpenTheatreStorage == undefined) {
                return defaultVal
            } else {
                if (window.OpenTheatreStorage[key] == undefined) {
                    return defaultVal
                } else {
                    return window.OpenTheatreStorage[key];
                }
            }
    ...
    function getEnableTextMessage() {
        return getOpenTheatreStorage('EnableTextMessage', true);
    }

    function getEnableMiniBar() {
        return getOpenTheatreStorage('EnableMiniBar', true);
    }

    function skipIntroLen() {
        try {
            let len = parseInt(window.OpenTheatreStorage.SkipIntroLength);
            if (window.OpenTheatreStorage.SkipIntro && !isNaN(len)) {
                return len;
            }
        } catch { }
        return 0;
    ...
    function isEasyShareEnabled() {
        ...
        try {
            ...
            return window.OpenTheatreEasyShare != 'disabled' && window.OpenTheatreStorage.EasyShare != false;
        } catch {
            return false;
        }
    }
    ...
    function isEasyShareMember() {
        try {
            return window.OpenTheatreEasyShareMemberSite == true;
        } catch {
            return false;
        }
    }
    ...
    function useMobileStyle(videoDom) {
        ...
        document.body.childNodes.forEach(e => {
            try {
                if (e != videoDom && e.style && e.id != 'OpenTheatreWrapper') {
    ...
    function isWeb() {
        try {
            let type = window.OpenTheatreStorage.UserscriptType;
    ...
    function select(query) {
        let e = window.openTheatreFlyPannel.wrapper.querySelector(query);
        return e;
    }
    ...
    function isRoomProtected() {
        try {
            return window.OpenTheatreStorage == undefined || window.OpenTheatreStorage.PasswordProtectedRoom != false;
        } catch {
            return true;
        }
    }
        ...
        async sendTextMessage(id, msg) {
            this.send({
                ...
                "data": {
                    "msg": msg,
                    "id": id,
                    "voiceId": getOpenTheatreStorage('PublicReechoVoiceId', "")
                }
            })
        },
    ...
    const Voice = {
        ...
        join: async function (name, rname, mutting = false) {
            ...
            try {
                cancellingNoise = !(window.OpenTheatreStorage.EchoCancellation === false);
            } catch { }
            ...
            async function rpc(method, params = [], retryTime = -1) {
                try {
                    const response = await window.openTheatreExtension.Fetch(extension.video_together_host + "/kraken", "POST", { id: generateUUID(), method: method, params: params }, {
    ...
    class OpenTheatreFlyPannel {
        constructor() {
            this.sessionKey = "OpenTheatreFlySaveSessionKey";
            this.isInRoom = false;
            ...
            setInterval(() => {
                if (getEnableMiniBar() && getEnableTextMessage() && document.fullscreenElement != undefined
                    ...
                    shadowWrapper.id = "OpenTheatrefullscreenSWrapper";
                    let wrapper;
                    try {
...
<div class="container" id="container">
    ...
    <button id="send-button">Send</button>
            ...
            if (this.isMain) {
                ...
                let shadowWrapper = document.createElement("div");
                shadowWrapper.id = "OpenTheatreWrapper";
...
<div id="openTheatreFlyPannel" style="display: none;">
  <div id="openTheatreHeader" class="vt-modal-header">
    <div style="display: flex;align-items: center;">
      ...
      <div class="vt-modal-title">OpenTheatre</div>
    </div>
    ...
    <a href="https://setting.2gether.video/" target="_blank" id="openTheatreSetting" type="button"
    ...
    <button id="openTheatreMinimize" type="button" aria-label="Close" class="vt-modal-close vt-modal-title-button">
      <span class="vt-modal-close-x">
  ...
  <div class="vt-modal-content">
    ...
    <div class="vt-modal-body">
      <div id="mainPannel" class="content">
        <div style="height: 22.5px;">
          <span id="openTheatreRoleText"></span>
          <span id="memberCount"></span>
        </div>
        <div id="openTheatreStatusText" style="height: 22.5px;"></div>
        <div style="margin-bottom: 10px;">
          <span class="ellipsis" id="openTheatreRoomNameLabel">Room</span>
          <input id="openTheatreRoomNameInput" autocomplete="off" placeholder="Name of room">
        </div>
        <div>
          <span class="ellipsis" id="openTheatreRoomPasswordLabel">Password</span>
          <input id="openTheatreRoomPdIpt" autocomplete="off" placeholder="Host's password">
        </div>
        <div>
    ...
    <div class="vt-modal-footer">
      ...
      <div id="lobbyBtnGroup">
        <button id="openTheatreCreateButton" class="vt-btn vt-btn-primary" type="button">
          <span>Create</span>
        </button>
        <button id="openTheatreJoinButton" class="vt-btn vt-btn-secondary" type="button">
          <span>Join</span>
        </button>
      </div>
      ...
      <div id="roomButtonGroup" style="display: none;">
        ...
        <button id="openTheatreExitButton" class="vt-btn vt-btn-dangerous" type="button">
          <span>Exit</span>
        </button>
      ...
      <button id="openTheatreHelpButton" class="vt-btn" type="button">
        <span>Help</span>
      </button>
    </div>
  </div>
</div>
<div style="width: 24px; height: 24px;" id="openTheatreSamllIcon">
  <img draggable="false" width="24px" height="24px" id="openTheatreMaximize"
    src="data:image/png;base64,">
  </img>
</div>
...
<style>
  ...
  #openTheatreFlyPannel #openTheatreHeader {
    cursor: move;
    touch-action: none;
    align-items: center;
    display: flex;
  ...
  #openTheatreRoomNameLabel,
  #openTheatreRoomPasswordLabel {
    display: inline-block;
    width: 76px;
  }

  #openTheatreRoomNameInput:disabled {
    border: none;
    background-color: transparent;
    color: black;
  }

  #openTheatreRoomNameInput,
  #openTheatreRoomPdIpt {
    width: 150px;
    height: auto;
    font-family: inherit;
    font-size: inherit;
  ...
  .ellipsis {
    ...
    text-overflow: ellipsis;
                ...
                wrapper.querySelector("#openTheatreMinimize").onclick = () => { this.Minimize() }
                wrapper.querySelector("#openTheatreMaximize").onclick = () => { this.Maximize() }
                ["", "webkit"].forEach(prefix => {
                    document.addEventListener(prefix + "fullscreenchange", (event) => {
                        if (document.fullscreenElement || document.webkitFullscreenElement) {
                            hide(this.openTheatreFlyPannel);
                            hide(this.openTheatreSamllIcon);
                        } else {
                            if (this.minimized) {
                                this.Minimize();
                            } else {
                ...
                this.createRoomButton = wrapper.querySelector('#openTheatreCreateButton');
                this.joinRoomButton = wrapper.querySelector("#openTheatreJoinButton");
                this.roomButtonGroup = wrapper.querySelector('#roomButtonGroup');
                this.exitButton = wrapper.querySelector("#openTheatreExitButton");
                this.callBtn = wrapper.querySelector("#callBtn");
                this.callBtn.onclick = () => Voice.join("", window.openTheatreExtension.roomName);
                this.helpButton = wrapper.querySelector("#openTheatreHelpButton");
                ...
                this.callErrorBtn.onclick = () => {
                    Voice.join("", window.openTheatreExtension.roomName);
                }
                ...
                this.exitButton.onclick = (() => {
                    window.openTheatreExtension.exitRoom();
                });
                this.openTheatreRoleText = wrapper.querySelector("#openTheatreRoleText")
                this.openTheatreSetting = wrapper.querySelector("#openTheatreSetting");
                hide(this.openTheatreSetting);
                this.inputRoomName = wrapper.querySelector('#openTheatreRoomNameInput');
                this.inputRoomPassword = wrapper.querySelector("#openTheatreRoomPdIpt");
                this.inputRoomNameLabel = wrapper.querySelector('#openTheatreRoomNameLabel');
                this.inputRoomPasswordLabel = wrapper.querySelector("#openTheatreRoomPasswordLabel");
                this.openTheatreHeader = wrapper.querySelector("#openTheatreHeader");
                this.openTheatreFlyPannel = wrapper.getElementById("openTheatreFlyPannel");
                this.openTheatreSamllIcon = wrapper.getElementById("openTheatreSamllIcon");

                this.volume = 1;
                this.statusText = wrapper.querySelector("#openTheatreStatusText");
                this.InLobby(true);
                this.Init();
                setInterval(() => {
                    this.ShowPannel();
            ...
            try {
                document.querySelector("#openTheatreLoading").remove()
            } catch { }
        }
        ...
        ShowTxtMsgTouchPannel() {
            ...
            touch.id = "openTheatreTxtMsgTouch";
            ...
            this.setTxtMsgTouchPannelText("OpenTheatre: You got a new message, click the screen to receive");
        }
        ...
        setTxtMsgTouchPannelText(s) {
            ...
            try {
                if (window.OpenTheatreStorage.PublicMessageVoice != undefined) {
                    voiceSelect.value = window.OpenTheatreStorage.PublicMessageVoice;
                } else {
        ...
        Minimize(isDefault = false) {
            ...
            hide(this.openTheatreFlyPannel);
            show(this.openTheatreSamllIcon);
        }
        ...
        Maximize(isDefault = false) {
            ...
            this.disableDefaultSize = true;
            show(this.openTheatreFlyPannel);
            hide(this.openTheatreSamllIcon);
        }

        SaveIsMinimized(minimized) {
            localStorage.setItem("OpenTheatreMinimizedHere", minimized ? 1 : 0)
        }

        Init() {
            let OpenTheatreMinimizedHere = localStorage.getItem("OpenTheatreMinimizedHere");
            if (OpenTheatreMinimizedHere == 0) {
                this.Maximize(true);
            } else if (OpenTheatreMinimizedHere == 1) {
                this.Minimize(true);
            }
        }
        ...
        CreateRoomButtonOnClick() {
            ...
            window.openTheatreExtension.CreateRoom(roomName, password);
        }
        ...
        JoinRoomButtonOnClick() {
            ...
            let roomName = this.inputRoomName.value;
            let password = this.inputRoomPassword.value;
            window.openTheatreExtension.JoinRoom(roomName, password);
        }
    ...
    class OpenTheatreExtension {
        ...
        constructor() {
            ...
            const messageListener = message => {
                ...
                if (message.data.context) {
                    ...
                    window.OpenTheatreStorage = message.data.context.OpenTheatreStorage;
                }
                ...
                try {
                    ...
                    setTimeout(() => {
                        ...
                        for (let i = 0; i < allDoms.length; i++) {
                            ...
                            if (cssObj.getPropertyValue("z-index") == 2147483647 && !allDoms[i].id.startsWith("openTheatre")) {
        ...
        async gotTextMsg(id, msg, prepare = false, idx = -1, audioUrl = undefined) {
            ...
            if (idx == -1) {
                try {
                    ssu.voice = speechSynthesis.getVoices().find(v => v.voiceURI == window.OpenTheatreStorage.PublicMessageVoice);
                } catch { }
            } else {
        ...
        setRole(role) {
            let setRoleText = text => {
                updateInnnerHTML(window.openTheatreFlyPannel.openTheatreRoleText, text);
            }
            this.role = role
            switch (role) {
        ...
        async generateEasyShareLink(china = false) {
            const path = `${language}/easyshare.html?OpenTheatreRole=3&OpenTheatreRoomName=${this.roomName}&OpenTheatreTimestamp=9999999999&OpenTheatreUrl=&OpenTheatrePassword=${this.password}`;
            if (china) {
        ...
        async Fetch(url, method = 'GET', data = null) {
            ...
            try {
                ...
                url.searchParams.set("loaddingVersion", window.OpenTheatreStorage.LoaddingVersion);
                url.searchParams.set("runtimeType", window.OpenTheatreStorage.UserscriptType);
            } catch (e) { }
            try {
                url.searchParams.set("userId", window.OpenTheatreStorage.PublicUserId);
            } catch (e) { }
            url = url.toString();
        ...
        async ForEachVideo(func) {
            try {
                ...
                if (window.location.hostname.endsWith("disneyplus.com")) {
                    try {
                        ...
                        if (ff && rr && video) {
                            if (!video.openTheatreVideoWrapper) {
                                video.openTheatreVideoWrapper = new VideoWrapper();
                            }
                            let videoWrapper = video.openTheatreVideoWrapper;
                ...
                if (window.location.hostname.endsWith("netflix.com")) {
                    try {
                        ...
                        if (!player.openTheatreVideoWrapper) {
                            player.openTheatreVideoWrapper = new VideoWrapper();
                        }
                        let videoWrapper = player.openTheatreVideoWrapper;
                ...
                if (window.location.host.includes('pan.baidu.com')) {
                    ...
                    if (this.BaiduPanPlayer) {
                        if (!this.BaiduPanPlayer.openTheatreVideoWrapper) {
                            this.BaiduPanPlayer.openTheatreVideoWrapper = new VideoWrapper();
                        }
                        let videoWrapper = this.BaiduPanPlayer.openTheatreVideoWrapper;
            ...
            try {
                ...
                if (window.__PLAYER__ != undefined) {
                    if (window.__PLAYER__.openTheatreVideoWrapper == undefined) {
                        window.__PLAYER__.openTheatreVideoWrapper = new VideoWrapper();
                    }
                    let videoWrapper = window.__PLAYER__.openTheatreVideoWrapper;
                    ...
                    videoWrapper.currentTimeSetter = (v) => { if (!videoWrapper.openTheatrePaused) { window.__PLAYER__.seek(v) } };
            ...
            this.video_tag_names.forEach(async tag => {
                ...
                for (let i = 0; i < videos.length; i++) {
                    try {
                        try {
                            if (videos[i].OpenTheatreDisabled) {
                                continue;
                            }
                        } catch { };
                        try {
            ...
            for (let i = 0; i < iframs.length; i++) {
                PostMessage(iframs[i].contentWindow, {
                    ...
                    context: {
                        ...
                        OpenTheatreStorage: window.OpenTheatreStorage,
        ...
        async testM3u8OrVideoUrl(testUrl) {
            ...
            return new Promise((res, rej) => {
                ...
                OpenTheatreFetch(testUrl, { signal: abortController.signal }).then(response => {
        ...
        UpdateStatusText(text, color) {
            if (window.self != window.top) {
                ...
                window.openTheatreFlyPannel.UpdateStatusText(text + "", color);
            }
        }
            ...
            switch (type) {
                ...
                case MessageType.SyncMasterVideo:
                    this.ForEachVideo(async video => {
                        if (video.OpenTheatreVideoId == data.video.id) {
                            try {
                ...
                case MessageType.UpdateStatusText:
                    window.openTheatreFlyPannel.UpdateStatusText(data.text, data.color);
                    break;
                ...
                case MessageType.SyncStorageValue: {
                    const firstSync = (window.OpenTheatreSettingEnabled == undefined)
                    window.OpenTheatreStorage = data;
                    if (!this.isMain) {
                        return;
                    }
                    try {
                        if (window.OpenTheatreStorage.PublicNextDownload.url == window.location.href
                            && this.HasDownload != true) {
                            const a = document.createElement("a");
                            a.href = window.OpenTheatreStorage.PublicNextDownload.url;
                            a.download = window.OpenTheatreStorage.PublicNextDownload.filename;
                            a.click();
                            this.HasDownload = true;
                        }
                    } catch { }
                    ...
                    if (!window.openTheatreFlyPannel.disableDefaultSize && firstSync) {
                        if (data.MinimiseDefault) {
                            window.openTheatreFlyPannel.Minimize(true);
                        } else {
                            window.openTheatreFlyPannel.Maximize(true);
                        }
                    }
                    ...
                    try {
                        if (firstSync) {
                            if (!isWeb()) {
                                window.openTheatreFlyPannel.openTheatreSetting.href = "https://videotogether.github.io/setting/v2.html";
                                show(select('#openTheatreSetting'));
                            } else {
                                // website
                                if (window.openTheatreWebsiteSettingUrl != undefined) {
                                    window.openTheatreFlyPannel.openTheatreSetting.href = window.openTheatreWebsiteSettingUrl;
                                    show(select('#openTheatreSetting'));
                                }
                            }
                        }
                    ...
                    window.OpenTheatreSettingEnabled = true;
                    break;
                }
        ...
        setActivatedVideoDom(videoDom) {
            if (videoDom.OpenTheatreVideoId == undefined) {
                videoDom.OpenTheatreVideoId = generateUUID();
            }
            sendMessageToTop(MessageType.ActivatedVideo, new VideoModel(videoDom.OpenTheatreVideoId, videoDom.duration, Date.now() / 1000, Date.now() / 1000));
        }
            ...
            let observer = new WebKitMutationObserver(function (mutations) {
                mutations.forEach(function (mutation) {
                    for (let i = 0; i < mutation.addedNodes.length; i++) {
                        ...
                        try {
                            if (extension.isMain && window.OpenTheatreStorage.OpenAllLinksInSelf != false && _this.role != _this.RoleEnum.Null) {
        ...
        RecoveryState() {
            function RecoveryStateFrom(getFunc) {
                let vtRole = getFunc("OpenTheatreRole");
                let vtUrl = getFunc("OpenTheatreUrl");
                let vtRoomName = getFunc("OpenTheatreRoomName");
                let timestamp = parseFloat(getFunc("OpenTheatreTimestamp"));
                let password = getFunc("OpenTheatrePassword");
                let voice = getFunc("OpenTheatreVoice");
                ...
                if (vtUrl != null && vtRoomName != null) {
                    if (vtRole == this.RoleEnum.Member || vtRole == this.RoleEnum.Master) {
                        ...
                        window.openTheatreFlyPannel.inputRoomName.value = vtRoomName;
                        window.openTheatreFlyPannel.inputRoomPassword.value = password;
                        window.openTheatreFlyPannel.InRoom();
                        switch (voice) {
                            case VoiceStatus.MUTED:
            ...
            if (window.OpenTheatreStorage != undefined && window.OpenTheatreStorage.OpenTheatreTabStorageEnabled) {
                try {
                    RecoveryStateFrom.bind(this)(key => window.OpenTheatreStorage.OpenTheatreTabStorage[key]);
                } catch { };
                return;
            }
            let localTimestamp = window.sessionStorage.getItem("OpenTheatreTimestamp");
            let urlTimestamp = url.searchParams.get("OpenTheatreTimestamp");
        ...
        exitRoom() {
            ...
            this.duration = undefined;
            window.openTheatreFlyPannel.inputRoomName.value = "";
            window.openTheatreFlyPannel.inputRoomPassword.value = "";
            this.roomName = "";
            this.setRole(this.RoleEnum.Null);
            window.openTheatreFlyPannel.InLobby();
        ...
        getVoiceVolume() {
            ...
            try {
                if (window.OpenTheatreStorage.OpenTheatreTabStorage.VoiceVolume != null) {
                    return window.OpenTheatreStorage.OpenTheatreTabStorage.VoiceVolume;
                }
            } catch { }
            return 100;
        }
        ...
        getVideoVolume() {
            ...
            try {
                if (window.OpenTheatreStorage.OpenTheatreTabStorage.VideoVolume != null) {
                    return window.OpenTheatreStorage.OpenTheatreTabStorage.VideoVolume;
                }
            } catch { }
            return 100;
        }
        ...
        async ScheduledTask(scheduled = false) {
            ...
            try {
                await this.ForEachVideo(video => {
                    if (video.OpenTheatreVideoId == undefined) {
                        video.OpenTheatreVideoId = generateUUID();
                    }
                    if (video instanceof VideoWrapper || video.OpenTheatreChoosed == true) {
                        // ad hoc
                        sendMessageToTop(MessageType.ReportVideo, new VideoModel(video.OpenTheatreVideoId, video.duration, 0, Date.now() / 1000, 1));
                    } else {
                        sendMessageToTop(MessageType.ReportVideo, new VideoModel(video.OpenTheatreVideoId, video.duration, 0, Date.now() / 1000));
                    }
                })
            ...
            if (this.role != this.RoleEnum.Null) {
                ...
                try {
                    if (this.isMain && window.OpenTheatreStorage.OpenAllLinksInSelf != false && !this.allLinksTargetModified) {
                    ...
                    case this.RoleEnum.Member: {
                        ...
                        if (isEasyShareMember()) {
                            if (isEmpty(room['m3u8Url'])) {
                                ...
                                newUrl = _url.href;
                                window.OpenTheatreEasyShareUrl = room['url'];
                                window.OpenTheatreEasyShareTitle = room['videoTitle'];
                            }
                        }
                        if (newUrl != this.url && (window.OpenTheatreStorage == undefined || !window.OpenTheatreStorage.DisableRedirectJoin)) {
                            if (window.OpenTheatreStorage != undefined && window.OpenTheatreStorage.OpenTheatreTabStorageEnabled) {
                                let state = this.GetRoomState(newUrl);
                                sendMessageToTop(MessageType.SetTabStorage, state);
                                setInterval(() => {
                                    if (window.OpenTheatreStorage.OpenTheatreTabStorage.OpenTheatreUrl == newUrl) {
                                        try {
                                            if (isWeb()) {
        ...
        linkWithoutState(link) {
            ...
            url.searchParams.delete("OpenTheatreUrl");
            url.searchParams.delete("OpenTheatreRoomName");
            url.searchParams.delete("OpenTheatreRole");
            url.searchParams.delete("OpenTheatrePassword");
            url.searchParams.delete("OpenTheatreTimestamp");
            return url.toString();
        }
        ...
        GetRoomState(link) {
            ...
            if (voice == VoiceStatus.CONNECTTING) {
                try {
                    voice = window.OpenTheatreStorage.OpenTheatreTabStorage.OpenTheatreVoice;
                } catch {
            ...
            return {
                OpenTheatreUrl: link,
                OpenTheatreRoomName: this.roomName,
                OpenTheatrePassword: this.password,
                OpenTheatreRole: this.role,
                OpenTheatreTimestamp: Date.now() / 1000,
                OpenTheatreVoice: voice,
        ...
        SaveStateToSessionStorageWhenSameOrigin(link) {
            ...
            try {
                ...
                if (link == "" || sameOrigin) {
                    window.sessionStorage.setItem("OpenTheatreUrl", link);
                    window.sessionStorage.setItem("OpenTheatreRoomName", this.roomName);
                    window.sessionStorage.setItem("OpenTheatrePassword", this.password);
                    window.sessionStorage.setItem("OpenTheatreRole", this.role);
                    window.sessionStorage.setItem("OpenTheatreTimestamp", Date.now() / 1000);
                    return sameOrigin;
                } else {
                    return false;
                }
        ...
        linkWithMemberState(link, newRole = undefined, expire = true) {
            ...
            url.search = "";
            url.searchParams.set("OpenTheatreUrl", link);
            url.searchParams.set("OpenTheatreRoomName", this.roomName);
            url.searchParams.set("OpenTheatrePassword", this.password);
            url.searchParams.set("OpenTheatreRole", newRole ? newRole : this.role);
            url.searchParams.set("OpenTheatreTimestamp", expire ? Date.now() / 1000 : 1e10);
        ...
        async SyncMemberVideo(data, videoDom) {
            ...
            if (paused == false) {
                videoDom.openTheatrePaused = false;
                ...
                videoDom.openTheatrePaused = true;
        ...
        setWaitForLoadding(b) {
            let enabled = true;
            try { enabled = (window.OpenTheatreStorage.WaitForLoadding != false) } catch { }
        ...
        EnableDraggable() {
            function filter(e) {
                ...
                if (window.openTheatreFlyPannel.openTheatreHeader.contains(e.target)) {
                    target = window.openTheatreFlyPannel.openTheatreFlyPannel;
                } else {
                    return;
                }


                target.openTheatreMoving = true;
                ...
                function endDrag() {
                    target.openTheatreMoving = false;
                }
                target.onmouseup = endDrag;
                target.ontouchend = endDrag;
                target.onpointerup = endDrag;
            }
            window.openTheatreFlyPannel.openTheatreHeader.onmousedown = filter;
            window.openTheatreFlyPannel.openTheatreHeader.ontouchstart = filter;
            window.openTheatreFlyPannel.openTheatreHeader.onpointerdown = filter;
        }
    }
    ...
    if (window.openTheatreFlyPannel === undefined) {
        window.openTheatreFlyPannel = null;
        try {
            var windowPannel = new OpenTheatreFlyPannel();
            window.openTheatreFlyPannel = windowPannel;
        } catch (e) { console.error(e) }
    }
    if (window.openTheatreExtension === undefined) {
        window.openTheatreExtension = null;
        var extension = new OpenTheatreExtension();
        window.openTheatreExtension = extension;
