...
async function $(page, selector) {
    return await page.evaluateHandle(`document.querySelector("#OpenTheatreWrapper").shadowRoot.querySelector("${selector}")`);
}
...
(async () => {
    ...
    let OpenTheatreChrome = "../../source/chrome";
    const browser = await puppeteer.launch({
        ...
        userDataDir: '.profile',
        args: [
            `--disable-extensions-except=${OpenTheatreChrome}`,
            `--load-extension=${OpenTheatreChrome}`,
            '--enable-automation'
        ]
        // args,
    });
    ...
    async function Test(url, name, adTime) {
        ...
        await sleep(1000);
        const HostNameInput = await $(hostPage, "#openTheatreRoomNameInput");
        await HostNameInput.type(roomName);
        const HostPasswordInput = await $(hostPage, "#openTheatreRoomPasswordInput");
        await HostPasswordInput.type(password);
        const openTheatreCreateButton = await $(hostPage, "#openTheatreCreateButton");
        await openTheatreCreateButton.click();
        for (let i = 0; i < 20; i++) {
        ...
        // await pageMember1.$eval('#openTheatreRoomNameInput', el => el.value = '');

        const MNameInput = await $(memberpage, "#openTheatreRoomNameInput");
        await MNameInput.type(roomName);
        const openTheatreJoinButton = await $(memberpage, "#openTheatreJoinButton");
        await openTheatreJoinButton.click();
