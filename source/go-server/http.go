...
func (h *slashFix) handleStaticCheckEasyShare(res http.ResponseWriter, req *http.Request) {
	h.Html(res, 200, "<script>let url=window.location.hash.substring(1);fetch(url).then(r => {if (r.ok) {window.top.postMessage({source: 'OpenTheatre',type: 25,data: {m3u8Url: url}}, '*');}})</script>")
}
