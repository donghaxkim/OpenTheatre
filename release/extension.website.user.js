...
(async function () {
    ...
    if (type == "website" || type == "website_debug") {
        ...
        getGM().getTab = async () => {
            let tab = sessionStorage.getItem('OpenTheatreTab');
            return tab == null ? {} : JSON.parse(tab);
        }

        getGM().saveTab = async (tab) => {
            return sessionStorage.setItem('OpenTheatreTab', JSON.stringify(tab));
        }
    ...
    async function AppendKey(key) {
        let keysStr = await getGM().getValue("OpenTheatreKeys");
        let keys = new Set(JSON.parse(keysStr));
        keys.add(key);
        await getGM().setValue("OpenTheatreKeys", JSON.stringify(Array.from(keys)));
    }
    ...
    async function GetKeys() {
        ...
        try {
            ...
            await getGM().setValue("OpenTheatreKeys", "[]");
            return [];
        }
    }
    ...
    async function SetTabStorage(data) {
        try {
            ...
            tabObj.OpenTheatreTabStorage = data;
            await getGM().saveTab(tabObj);
            window.postMessage({
                source: "OpenTheatre",
                type: 19,
                data: tabObj.OpenTheatreTabStorage
            })
        } catch (e) { };
    }

    if (window.OpenTheatreLoading) {
        return;
    }
    window.OpenTheatreLoading = true;
    ...
    async function PostStorage() {
        try {
            ...
            data["OpenTheatreTabStorageEnabled"] = true;
            data['ExtensionLanguages'] = languages;
            try {
                data["OpenTheatreTabStorage"] = (await getGM().getTab()).OpenTheatreTabStorage;
            } catch (e) {
                data["OpenTheatreTabStorageEnabled"] = false;
            }
            window.postMessage({
                source: "OpenTheatre",
                type: 16,
    ...
    wrapper.innerHTML = `<div id="openTheatreLoading">
    <div id="openTheatreLoadingwrap">
...
<style>
    ...
    #openTheatreLoadingwrap {
        width: 100%;
        display: flex;
        align-items: center;
        justify-content: center;
    }
    #openTheatreLoadingwrap img {
        margin-right: 12px;
    }
    #openTheatreLoadingwrap a {
        color: #212529;
        text-decoration: none;
    }
    #openTheatreLoadingwrap a:hover {
        color: #1890ff;
    ...
    if (isWebsite) {
        (function () {
            ...
            for (const iframe of iframes) {
                try {
                    if (iframe.contentWindow.OpenTheatreParentInject != true &&
                        ...
                        iframe.contentWindow.OpenTheatreParentInject = true;
                    }
                } catch (error) {
                }
            }
...
//*/
    ...
    function filter(e) {
        ...
        if (target.id != "openTheatreLoading") {
            return;
        }
