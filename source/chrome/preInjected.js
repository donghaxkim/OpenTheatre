window.OpenTheatreFetch = window.fetch;
...
(() => {
    ...
    if (window.OpenTheatrePreinjected) {
        return;
    }
    window.OpenTheatrePreinjected = true;

    setTimeout(() => {
        if (sessionStorage.getItem("OpenTheatreSuperEasyShare") === 'true') {
            console.log("OpenTheatreSuperEasyShare");
    ...
    function processResponseText(url, textContent) {
        if (isM3U8(textContent)) {
            ...
            if (window.OpenTheatreEasyShare === 'disabled') {
                window.OpenTheatreM3u8Files = null;
            } else {
                window.OpenTheatreM3u8Files = m3u8Files;
            }
